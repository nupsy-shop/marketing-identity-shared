/**
 * entra-id:flag_inactive_identity
 *
 * Drift remediation for HUMAN_INTERACTIVE identities whose provider account
 * exists in the Entra ID directory cache (is_present_in_idp=true) but is
 * admin-disabled (is_active=false). Marks the identity as `inactive`,
...
export const flagInactiveIdentityHandler: RemediationActionHandler = async (
  ...
  return flagInactiveIdentity('entra-id', params, context, instance);
};
