/**
 * Shared util: flag a HUMAN_INTERACTIVE identity as missing in a provider.
 *
 * Used by both `google-workspace:flag_missing_identity` and
 * `entra-id:flag_missing_identity`. Plugin handlers are thin wrappers that
 ...
 *   2. Updates `provisioning_status = 'not_found'` and sets
 *      `provisioning_providers_status[providerKey].status = 'NOT_FOUND'` with
 *      a timestamp, while preserving all other provider entries.
 *   3. Publishes an `identity.flagged_missing` audit event.
...
export async function flagMissingIdentity(
  ...
  if (!principalId) {
    throw new Error(`${providerKey}:flag_missing_identity: missing principalId`);
  }
  ...
  if (!identity) {
    throw new Error(
      `${providerKey}:flag_missing_identity: identity ${principalId} not found`,
    );
  }

  // Merge-update: preserve all other provider entries, only set ours to NOT_FOUND.
  ...
  publishAuditEvent({
    eventType: 'identity.flagged_missing',
    source: 'accesshive',
  ...
  dispatchNotification(instance.agency_id, 'identity.flagged_missing', {
    identityId: identity.id,
    identifier: identity.identifier,
    providerKey,
    title: 'Identity flagged as missing',
    body: `Identity ${identity.identifier} could not be found in ${providerKey}. Manual review required.`,
    severity: 'warning',
  }).catch(() => {});

  return {
    actionCompleted: true,
    actionType: `${providerKey}:flag_missing_identity`,
